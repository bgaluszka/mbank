...
class Mbank
{
	...
	public function __construct($countryCode = 'pl')
	{
		...
		$this->opts = array(
			...
			// cURL 8.x requires COOKIEFILE to activate cookie engine for sending
			CURLOPT_COOKIEFILE      => '',
		);
	...
	public function __destruct()
	{
        ...
        /**
	 * Starts mBank session using OAuth-based SCA login flow.
	 *
	 * The flow: GET login page -> follow OAuth authorize -> prelogin with credentials
	 * -> SCA device trust check -> obtain two-factor code -> finallogin
	 * -> follow OAuth callback -> extract verification token.
	 *
	 * @param string $username
	 * @param string $password
	 * @param string $dfp    Device fingerprint value (generated by browser JS)
	 * @param string $mbank8 mBank8 cookie value (persistent device identifier)
	 *
	 * @return bool
	 *
	...
	{
		$tld = ($this->countryCode === 'cs') ? 'cz' : $this->countryCode;
		$jsonHeaders = array('Content-Type: application/json');

		curl_setopt($this->curl, CURLOPT_COOKIELIST,
			".mbank.{$tld}\tTRUE\t/\tTRUE\t0\tmBank8\t{$mbank8}"
		);

		// Step 1a: Get login page (returns JS redirect to OAuth authorize URL)
		...
		if (!is_string($body) || !preg_match("/window\\.location\\.href\\s*=\\s*['\"]([^'\"]+)['\"]/", $body, $m)) {
			throw new \RuntimeException('login() failed - could not parse authorize URL from login page');
		}

		$authorizeUrl = $m[1];
		if (strpos($authorizeUrl, 'http') !== 0) {
			$authorizeUrl = $this->url . $authorizeUrl;
		}

		// Step 1b: Follow OAuth authorize flow (sets LoginContext cookie with returnUrl)
		curl_setopt_array($this->curl, array(
		...
		// Extract returnUrl from LoginContext cookie
		$returnUrl = null;
		foreach (curl_getinfo($this->curl, CURLINFO_COOKIELIST) as $cookie) {
			$parts = explode("\t", $cookie);
			if (count($parts) >= 7 && $parts[5] === 'LoginContext') {
				$contextJson = json_decode(urldecode($parts[6]), true);
				if (isset($contextJson['returnUrl'])) {
					$returnUrl = $contextJson['returnUrl'];
				}
				break;
			}
		}
		...
		if (!$returnUrl) {
			throw new \RuntimeException('login() failed - could not obtain returnUrl from LoginContext cookie');
		}

		curl_setopt($this->curl, CURLOPT_FOLLOWLOCATION, false);

		// Step 2: Send credentials (prelogin)
		$this->curl(array(
			CURLOPT_URL        => $this->url . '/signin/connect/api/users/prelogin',
			CURLOPT_POST       => true,
		...
		// Step 3: SCA device trust check
		$scaResponse = $this->curl(array(
			CURLOPT_URL        => $this->url . '/signin/connect/api/sca',
			CURLOPT_POST       => true,
			CURLOPT_POSTFIELDS => json_encode(array('dfp' => $dfp)),
			CURLOPT_HTTPHEADER => $jsonHeaders,
		));

		$scaToken = isset($scaResponse['token']) ? $scaResponse['token'] : null;

		// Step 3b: If SCA did not return a token, obtain one via finallogin probe
		if (!$scaToken) {
			$scaToken = $this->obtainTwoFactorCode($jsonHeaders, $returnUrl, $dfp);
		}

		// Step 4: Complete login with two-factor code
		$finalLoginData = array(
			'token'     => $scaToken,
			'returnUrl' => $returnUrl,
			'dfp'       => $dfp,
			'language'  => ucfirst($this->countryCode),
		);

		curl_setopt_array($this->curl, array(
			CURLOPT_URL        => $this->url . '/signin/connect/api/users/finallogin',
			CURLOPT_POST       => true,
			CURLOPT_POSTFIELDS => json_encode($finalLoginData),
			CURLOPT_HTTPHEADER => $jsonHeaders,
		) + $this->opts);

		$finalResponse = curl_exec($this->curl);
		$finalCode = curl_getinfo($this->curl, CURLINFO_HTTP_CODE);

		if ($finalCode !== 200) {
			throw new \RuntimeException('login() failed - finallogin returned HTTP ' . $finalCode);
		}

		$finalData = is_string($finalResponse) ? json_decode($finalResponse, true) : null;
		$callbackUrl = isset($finalData['returnUrl']) ? $finalData['returnUrl'] : $returnUrl;

		if (isset($finalData['tabId'])) {
			$this->tab = $finalData['tabId'];
		}

		// Step 5: Follow OAuth callback to establish session cookies
		curl_setopt_array($this->curl, array(
			CURLOPT_URL            => $this->url . $callbackUrl,
			CURLOPT_FOLLOWLOCATION => true,
			CURLOPT_POST           => false,
			CURLOPT_HTTPHEADER     => array(),
		) + $this->opts);
		...
		// Update tabId from cookie if available
		foreach (curl_getinfo($this->curl, CURLINFO_COOKIELIST) as $cookie) {
			$parts = explode("\t", $cookie);
			if (count($parts) >= 7 && $parts[5] === 'mBank_tabId') {
				$this->tab = $parts[6];
				break;
			}
		}

		// Step 6: Load main page and extract verification token
		...
		if (empty($this->token)) {
			throw new \RuntimeException('login() failed - could not extract verification token from main page');
		}
	...
	/**
	 * Obtains two-factor code when SCA does not return a token directly.
	 *
	 * Sends an empty token to the finallogin endpoint; the server responds
	 * with HTTP 422 including the valid two-factor code in the error body.
	 *
	 * @param array  $jsonHeaders HTTP headers for JSON requests
	 * @param string $returnUrl   OAuth return URL
	 * @param string $dfp         Device fingerprint
	 *
	 * @return string Two-factor code
	 *
	 * @throws \RuntimeException
	 */
	private function obtainTwoFactorCode(array $jsonHeaders, $returnUrl, $dfp)
	{
		curl_setopt_array($this->curl, array(
			CURLOPT_URL        => $this->url . '/signin/connect/api/users/finallogin',
			CURLOPT_POST       => true,
		...
		if (isset($data['errors']['validCode'][0])) {
			return $data['errors']['validCode'][0];
		}

		throw new \RuntimeException(
			'login() failed - could not obtain two-factor code. ' .
	...
	protected function curl(array $opts = array())
	{
		...
		// PHP 8: curl_exec() returns false on failure, json_decode(false) throws TypeError
		if (is_string($response) && ($json = json_decode($response, true))) {
			$response = $json;
		} else {
			$response = is_string($response) ? array($response) : array();
		}
